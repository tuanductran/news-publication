...
$archive_details = npub_current_archive_details();
?>

<div class="container">
<div class="row">
<div class="col">
<blockquote class="block-heading"><h1><?php echo esc_html( $archive_details['title'] ); ?></h1>
<?php
if ( ! empty( $archive_details['description'] ) ) {
	echo sprintf( '<cite>For: %s</cite>', esc_html( $archive_details['description'] ) );
}
?>
...
<div class="container px-2 latest-nbolck npub-big-title category-section npub-full-row">
	...
	if ( have_posts() ) :
		while ( have_posts() ) :
			...
			$post_cats  = get_the_category_list( ' , ', '', get_the_ID() );
			$post_title = get_the_title();
			$author_id  = get_post_field( 'post_author', get_the_ID() );
			?>
			<div class="row one-col-article p-2 my-1 px-sm-2 py-sm-2 my-sm-1 py-md-3 px-lg-2 px-md-3 my-md-1 py-lg-2 px-xl-2 py-xl-2 my-lg-3 d-flex align-items-lg-center align-items-md-top align-items-sm-start align-items-start check-in-view">
				...
				<div class="col-lg-7 col-md-7 col-sm-8 col-7 p-0 ps-lg-2 ps-md-3 ps-2 ps-sm-2 pt-xs-2 pt-sm-0">
					...
					?>
					<p class="full-width-1col-exerpt my-0 my-md-2 npub-text-color npub-blog-detail"><?php echo wp_kses_post( substr( get_the_excerpt(), 0, 150 ) ); ?></p>
				</div>
			</div>
			<?php
		endwhile;
