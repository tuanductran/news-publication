...
if (class_exists('WP_Customize_Control')) {
    class Npub_Range_Control extends WP_Customize_Control
    {
        ...
        {
            wp_enqueue_script(
                'npub-customizer-range-value-control',
                NPUB_THEME_CUSTOMIZER_URI . 'js/control-range-slider.js',
                ['jquery'],
                NPUB_THEME_VERSION,
                true
            );
            wp_enqueue_style(
                'npub-customizer-range-value-control',
                NPUB_THEME_CUSTOMIZER_URI . 'css/control-range-slider.css',
                [],
                NPUB_THEME_VERSION,
                'all'
            );
        ...
        $this->label
				...
				<input class="range-slider__range" type="range" value="<?php echo esc_attr(
        $this->value()
    ); ?>" 
					<?php
     $this->input_attrs();
