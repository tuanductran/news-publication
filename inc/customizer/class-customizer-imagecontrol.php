...
if ( class_exists( 'WP_Customize_Control' ) ) {
	...
	class Npub_Image_Select_Control extends WP_Customize_Control {
		...
		public function enqueue() {
			wp_enqueue_script( 'npub-image-select-control', NPUB_THEME_CUSTOMIZER_URI . '/js/control-image-select.js', array( 'jquery' ), NPUB_THEME_VERSION, true );
			wp_enqueue_style( 'npub-image-select-control', NPUB_THEME_CUSTOMIZER_URI . '/css/control-image-select.css', array(), NPUB_THEME_VERSION, 'all' );
		}
		...
		public function to_json() {
			...
			foreach ( $this->choices as $value => $args ) {
				$this->choices[ $value ]['url'] = esc_url( sprintf( $args['url'], NPUB_THEME_CUSTOMIZER_URI . '/images/' ) );
			}
