...
get_header();
?>
<div id="post-<?php the_ID(); ?>" <?php post_class( 'container pbody px-0 px-sm-2 px-md-2 px-lg-0' ); ?>>
...
<div class="container">
	<div class="post-metadata npub-extra-big-title">
		...
		<p class="d-inline pe-2 npub-secondary-color npub-secondary-title"><?php wp_kses_post( the_author() ); ?></p>
		<p class="d-inline p-0 npub-secondary-color npub-secondary-title"><?php echo wp_kses_post( get_the_date( 'D, M j' ) ); ?></p>
	</div>
</div>	
	<div class="container">
			<?php if ( has_post_thumbnail() ) : ?>
				<div class="img-div">
					<span class="size-npub-grande img-stylized">
						<?php echo get_the_post_thumbnail( get_the_ID(), '', array( 'class' => 'img-stylized' ) ); ?>	
					</span>
				</div>
			<?php endif; ?>
	</div>
...
<div class="container">
	<div class="row">
		...
		<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 sticky-side-disabled sidebar-cont pe-lg-0">
			<?php
			if ( is_active_sidebar( 'single_article_side_widget_area' ) ) {
	...
	<div class="related_posts">
		...
		if ( ! empty( $rel_cats ) && is_array( $rel_cats ) ) {
			...
			echo wp_kses_post(
				do_blocks(
					'<!-- wp:nblock/post-filter {"categories":[' . implode( ',', $rel_cats ) . '],"ppp":4,"layout":"4col-masonry"} --><!-- /wp:nblock/post-filter -->'
				)
			);
		}
		?>
